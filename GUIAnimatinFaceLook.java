...
public class GUIAnimatinFaceLook {
	...
	public void makeMouth(Graphics g, int mouseSize, int slope) {
		int xMiddle = xStart + h / 2;
		int yMiddle = yStart + 3 * w / 4;
		g.drawLine(xMiddle - mouseSize / 2, yMiddle, xMiddle ,yMiddle+slope);
        g.drawLine(xMiddle,yMiddle+slope,xMiddle+mouseSize/2,yMiddle);
	}

    public void makeBlow(Graphics g, int slope1){
        g.drawLine(xStart+w/6,yStart+h/6,xStart+(5*w/6)/2,yStart+h/6+slope1);
        g.drawLine(xStart+(7*w/6)/2,yStart+h/6+slope1,xStart+11*w/12,yStart+h/6);
    }
}
